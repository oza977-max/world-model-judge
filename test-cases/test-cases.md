...
least one case below. 68 cases total. Depth varies deliberately: single
...
**TC-WD6-01** · scenario test · judged
Given each world's declared tight-tolerance control task and loose-tolerance planning task,
When a reader is shown a trust horizon number with no other context,
Then they cannot tell what it means without also being told which task it belongs to — confirming WD-6/JU-7's task-relativity is enforced by the reporting format, not just asserted in prose.
...
**TC-MU2-02** · scenario test · executable
Given the two baselines run on both worlds,
When their one-step skill scores are computed,
Then both baselines score below any competent model under test — a sanity floor confirming "beats nothing-changes" is a real, non-trivial bar.
...
**TC-MU9-01** · scenario test · judged
Given a new model implementing only the MU-1 interface,
When it is added to the harness,
Then no change is required to the judge, the worlds, or the reporting layer — verified by diffing the repository before and after the new model's own file(s) are added.
...
**TC-JU8-03** · scenario test · executable
Given a model with padded, overly wide confidence ranges (near-zero exception rate),
When JU-8 runs,
Then it is flagged as a fault via the sharpness cross-check (JU-5), not silently rewarded for having "too few" exceptions.

**TC-JU9-01** · scenario test · executable
Given a completed judging run,
When the verdict record is produced,
Then it contains all six required fields (skill scores, error-vs-horizon with divergence benchmark, calibration+sharpness in/out of region, exception counts vs. thresholds, per-task trust horizons, not-tested list) in one structured record — the run fails rather than emitting a partial record if any field cannot be computed.

**TC-JU10-01** · scenario test · executable
Given any verdict,
When its limitations statement is inspected,
Then all six required disclosures are present: middle-of-distribution-only validity, toy-not-field, same-author blind-not-independent, borrowed-mechanism-not-full-SR-11-7, thresholds-are-modelling-choices, and passive-prediction-not-action-selection (the LeCun-review finding).
...
Given the committed thresholds/bands file,
When the first judging run against an unrigged model is attempted,
Then the run reads the thresholds from that file and refuses to proceed if the file's commit postdates the run (mirrors TC-MU6-01's mechanism, applied to JU-11 specifically).
...
**TC-NF2-01** · scenario test · judged
Given the full result set,
When run on an ordinary laptop,
Then wall-clock time stays under `<spec-value>` (Open Question 5) — flagged as pending until the technical spec fixes the number.

**TC-NF3-01** · scenario test · executable
Given the project's dependency manifest,
When compared against the technical spec's named minimal package list,
Then no dependency outside that named list is present.

**TC-NF4-01** · scenario test · executable
Given the full repository text,
When scanned for a maintained list of forbidden terms (employer name, internal team/committee names),
...
| WD-8 | — (Won't; nothing to verify) | JU-9 | TC-JU9-01 |
| MU-1 | TC-MU1-01, -02 | JU-10 | TC-JU10-01, -02 |
| MU-2 | TC-MU2-01, -02 | JU-11 | TC-JU11-01 |
...
| | | RP-8 | TC-RP8-01 |
| NF-1 | TC-NF1-01, -02 | NF-4 | TC-NF4-01 |
...
**Totals:** 68 test cases across 45 requirements. 5 negative/phantom-gate cases (WD-3, WD-7, NF-1 ×1 each, plus the two paired positive/negative pairs). 4 property-based cases (JU-1, JU-4, JU-5, JU-12, NF-6). 5 cases marked *judged* rather than *executable* (WD-6-01, MU-6-02, MU-9-01, JU-10-02, RP-5-01, NF-2-01, NF-5-01 — plain-language and process-discipline checks that need a reader, not a script). 3 cases explicitly pending an Open Question being fixed by the technical spec (JU-8-02, NF-2-01) rather than guessed at here.
