...
**TC-MU3-04 (action-response check + action-blind fixture)** (new, design-review-009 B1) · scenario test · executable
Given the action-blind fixture (a model that ignores its action input) and a genuine action-responsive model,
When each is given one starting state and two different actions drawn from the training region and the harness's action-response check is applied,
Then the action-blind fixture is flagged (its two predictions are identical whichever action it was given) and the responsive model passes (its two predictions differ by more than floating-point noise for at least one probe) — proving the harness checks that a *model* under test actually responds to its action, the one-layer-up analogue of TC-WD2-01's world-lever check, and that the fixture does its one job (labelled fixture, never a finding — MU-4/RP-8). The pass/fail rule is the one fixed in MU-3 before any model was built (MU-6/JU-11).
