...
Version 1.2 · 30 August 2026 · Derived from Requirements v1.2 and Test Cases v1.0 (design-review fixes applied 25 and 30 Aug 2026)
...
- `tests/gates/test_import_graph.py` walks `wmj.judge`'s AST and fails on any of three things (TC-NF6-01) — **design-review-002 broadened this from one check to three, after Round 2's Security and Reproducibility panels independently found the same gap from two angles: an `import wmj.*` statement (the original check); an `Import`/`ImportFrom` of a fixed ambient-access module list (`os`, `sys`, `time`, `datetime`, `socket`, `pathlib`, `subprocess`) anywhere in the judge's AST, since these can leak the same non-determinism/impurity an ambient read causes without ever importing another `wmj` package; and a `Call` node whose target is `importlib.import_module`, `__import__`, or `importlib.__import__`, since a dynamic import reaches another package's code without ever appearing as an `Import`/`ImportFrom` node.** All three checks live in the same AST walk and the same gate; a violation of any one fails the run. This is a build gate, not a convention.
...
- `wmj.models` exposes one registry; adding a model is one new file registering itself. **The zero-diff-outside-own-file property (TC-MU9-01) has a named mechanism, not just an assertion (design-review-002 fix — Round 2 found this was the one gate in this list with no described enforcement, unlike every other):** `tests/gates/test_registry_isolation.py` stages a throwaway fixture model file into a scratch git worktree checked out at the current commit, commits it in that worktree, and asserts `git diff --stat` between the two commits touches exactly one path under `wmj/models/` and nothing under `wmj/judge/`, `wmj/worlds/`, or `wmj/reporting/` — the same git-plumbing-based technique as the NF-4 scan below, applied to a diff instead of a content search.
...
| `JudgedResult` | the harness-owned envelope `{model_ref, model_name, is_fixture, verdict, meta}` — one per (model, world); wraps a `Verdict` with the identity/metadata facts only the harness holds (design-review addition, judge spec v1.2 §5) | harness | reporting, `out/verdicts/*.json` |
...
- `scan_repository()` reads `confidentiality_terms.local.txt` if present, plus the small set of structural pattern classes that are safe to keep generic and committed (e.g., a regex for common internal-only URL/hostname shapes). It walks every tracked file in the working tree (via `git ls-files`, so it naturally excludes `out/` and other gitignored paths) **and, design-review-002 fix, also scans commit-message history** (`git log --format=%B --all`, since a forbidden term typed into a commit message and later removed from file content remains permanently visible in `git log` on a public repo — Round 2's Security panel found the v1.1 scan never covered this channel), case-insensitively searching for each term, and fails loudly (per the Error-Handling Conventions above) listing every match's file/commit and location if any are found.
...
- **The residual gap, named rather than hidden (matching this project's own established pattern — models spec ADR-M5's git-rewritability disclosure):** CI cannot read the gitignored local file, so CI's own re-run of `scan_repository()` only ever has the committed structural patterns to check against — the real terms are enforceable only locally, by the one author who holds them, via the pre-commit hook. A commit made with `git commit --no-verify`, or from a fresh clone before the local terms file is configured, bypasses this control entirely. No technical mechanism in a single-author, no-server-side-review project fully closes this without infrastructure this toy-scale project doesn't otherwise need; it is disclosed here rather than presented as solved.
...
| ADR-003 structure | NF-6, JU-1, JU-12, MU-9 | TC-NF6-01, TC-JU1-01, TC-MU9-01, TC-JU12-01 |
