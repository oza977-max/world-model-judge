...
Version 1.7 · 31 August 2026 · Domain 3 of Requirements v1.2 · References: cross-cutting spec v1.7, worlds spec v1.4, models spec v1.7
...
**What TC-JU12-01 does not catch — disclosed (design-review-007, BC-4 confirmed; TC-JU12-04 documents it).** The guarded set is a *finite enumeration of the known ambient API surface*; no finite enumeration closes Python's reflection/syscall graph. Round 7 executed three uncatchable routes: `ctypes` (already resident via numpy) reaching raw syscalls with no import to see; an unseeded `numpy.random.Generator(numpy.random.PCG64())` drawing OS entropy via `getrandom()` (and `Generator.__init__` is a C immutable type — unpatchable); and any capability bound to a local name before the harness installs, beyond the enumerated holders. The first and third require deliberate intent (excluded by the threat model); the second and any accidental slip that varies between runs are caught by **the ten-run byte-identity gate (TC-NF1-01)**, which is the actual reproducibility guarantee. The static AST lint (TC-NF6-01/02) remains a fast pre-check. JU-12 is delivered by the tripwire and byte-identity together, residual named — not by the tripwire alone.
...
| Purity under blocked environment — effects guarded, reflection-route escape caught; numpy C-clock guarded; residual disclosed | TC-JU12-01, TC-JU12-02 (phantom-gate, design-review-005), TC-JU12-03 (numpy C-clock phantom-gate, design-review-006), TC-JU12-04 (disclosed residual, design-review-007) |
