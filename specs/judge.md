...
Version 1.8 · 31 August 2026 · Domain 3 of Requirements v1.2 · References: cross-cutting spec v1.8, worlds spec v1.4, models spec v1.8

> **Change note (v1.8 — design-review-008 repair, execution-verified).** §4's purity-enforcement body updated to match cross-cutting ADR-002 rule 3's generalized guards: `os.environ` patched at the class, `numpy.random` at both module and private-submodule level, `numpy.datetime64`'s rebind disclosed as the one structural exception. The disclosed residual narrows (Round 8 executed the submodule fix closing the `getrandom()`-via-`Generator(PCG64())` route Round 7 had disclosed; §8's TC-JU12-04 reference updated to match). §5's envelope write ownership corrected: reporting's `wmj/reporting/writer.py` writes `out/verdicts/*.json`, not the harness directly (design-review-008 C8 — the harness assembles and hands off). No Verdict schema change. See design-review-008.html.

> **Change note (v1.7 — design-review-007 repair, execution-verified).** §4's purity-enforcement body — which had carried the disproven `sys.modules`-swap wording through v1.6 (only the v1.6 changelog claimed the mutate-in-place fix; Round 7 found the body untouched) — is rewritten to match cross-cutting ADR-002 rule 3: mutate-in-place over the known ambient surface (`os.environ`'s real `_Environ` methods, all public `numpy.random` module names, `numpy.datetime64`), the "complete" claim dropped in favour of tripwire + TC-NF1-01 byte-identity backstop, with the `ctypes`/`getrandom` residual disclosed (BC-4, TC-JU12-04). TC-JU12-03 (numpy C-clock) and TC-JU12-04 added to §8. No Verdict schema change. See design-review-007.html.
...
**Purity enforcement — TC-JU12-01 is the primary tripwire; the byte-identity gate (TC-NF1-01) is the load-bearing backstop (design-review-007 repair, execution-verified — the mechanism below replaces the earlier `sys.modules`-swap wording this section carried through v1.6, which Round 6 executed and disproved, and drops the "complete" claim, which Round 7 executed and disproved; the authoritative mechanism is cross-cutting ADR-002 rule 3, and this section now matches it).** TC-JU12-01 runs the judge's full call graph — every public function, on a representative real `JudgeInput` — inside a context manager that **mutates each *known* ambient capability's real shared object in place** so it raises `AmbientAccessError` on use, so an impure operation through a documented API fails at the point of its *effect*, whatever route reached that object:
- `os`: `setattr(os, name, raise)` for the effectful functions (`system`, `getcwd`, `getenv`, `urandom`, `getpid`, `listdir`, `stat`, …), and **`os.environ` guarded by mutating the real `_Environ` instance's own bound methods in place** (not by rebinding `os.environ` — Round 7 proved a rebind is defeated by a reference captured before install);
- `builtins.open`, `io.open`; `time`, `datetime` clock entry points; `socket` / any network entry; `subprocess`;
- **randomness:** **every public callable in `numpy.random` is patched at the module level** (Round 7 enumerated 63 — `normal`, `uniform`, `rand`, `randn`, `choice`, …), not just `default_rng`/`Generator` — numpy binds the legacy globals as module-level names at import, so patching only the two named constructors leaves `np.random.normal()` live (executed); the judge needs no randomness (JU-12) and the run asserts it consumed none;
- **numpy's C clock:** `numpy.datetime64` is patched at module level (Round 6 executed `np.datetime64('now')` reading the wall clock while Python's `time` was guarded).

Because the guards mutate the real shared objects, a reflection route that recovers `os` via `<fn>.__globals__['os']` ends at the same mutated object and raises (TC-JU12-02); the `numpy.datetime64` read raises (TC-JU12-03). The test asserts the judge produces the correct verdict from its arguments alone and no guard fired.

**What TC-JU12-01 does not catch — disclosed and narrowed (design-review-008, BC-4 confirmed; TC-JU12-04 documents it).** The guarded set is a *finite enumeration of the known ambient API surface*; no finite enumeration closes Python's reflection/syscall graph. Round 8 executed the guards patched at every level each object is bound (module *and* private submodule for `numpy.random`) and found this closes the `getrandom()`-via-unseeded-`Generator(PCG64())` route Round 7 had disclosed as uncatchable — construction now raises at either binding before any entropy syscall runs. Two routes remain, both requiring deliberate intent: `ctypes` (already resident via numpy) reaching raw syscalls with no import to see; and any capability bound to a local name before the harness installs, beyond the enumerated holders (the one guard this structurally applies to is `numpy.datetime64`'s disclosed rebind, cross-cutting ADR-002 rule 3). Both are excluded by the threat model. Any accidental slip that still varies between runs — including through routes this enumeration failed to name — is caught by **the ten-run byte-identity gate (TC-NF1-01), run across separate processes**, which is the actual reproducibility guarantee. The static AST lint (TC-NF6-01/02) remains a fast pre-check. JU-12 is delivered by the tripwire and byte-identity together, residual named — not by the tripwire alone.
...
The harness constructs one envelope per (model, world) immediately after calling the judge and hands it to reporting; this envelope — never the bare `Verdict` — is what reporting's `wmj/reporting/writer.py` writes to `out/verdicts/*.json` (via the canonical serializer, cross-cutting ADR-002 rule 4 — design-review-008 C8: reporting is the sole writer of every artefact under `out/`, matching the container diagram; the harness assembles, it does not write). `model_ref` is an opaque index; the judge never sees a name or a fixture flag at any point in its call graph. **All nine JU-9 field groups are mandatory inside the pure `Verdict`** (design-review-002 made it eight; design-review-006 adds `limitations` as the ninth — Round 6 found judge.md's own "eight" enumeration excluded `limitations` while reporting.md's model card called it "one of the eight mandatory groups"; since `limitations` is the JU-10 honesty field the whole project treats as load-bearing, it must be refuse-on-missing like the rest, not a silently-optional field): skill, error-vs-horizon, calibration+sharpness, exceptions, **trials**, **climatology**, trust horizons, not-tested, **limitations**. `verdict.py` raises rather than emitting a record with any of the nine missing or null (TC-JU9-01, TC-JU9-02). (`limitations` is a hardcoded constant tuple, ADR-J7, so it will not in practice go missing — but the refuse-guard now covers it by contract, matching reporting.md's consumer-side claim.)
...
| Purity under blocked environment — effects guarded at every reachable binding, reflection-route escape caught; numpy C-clock guarded; residual disclosed and narrowed | TC-JU12-01, TC-JU12-02 (phantom-gate, design-review-005), TC-JU12-03 (numpy C-clock phantom-gate, design-review-006), TC-JU12-04 (disclosed residual, design-review-007; narrowed design-review-008) |
...
| 1.7 | 2026-08-31 | Design-review-007 repair (execution-verified): §4 purity-enforcement *body* rewritten to mutate-in-place (it had kept the disproven `sys.modules`-swap wording through v1.6 — only the changelog had claimed the fix), the "complete" claim dropped for tripwire + TC-NF1-01 byte-identity backstop with the `ctypes`/`getrandom` residual disclosed (BC-4); guards `os.environ`'s real `_Environ` methods and all public `numpy.random` module names and `numpy.datetime64`; TC-JU12-03/-04 added to §8. |
| 1.8 | 2026-08-31 | Design-review-008 repair (execution-verified): §4 guards generalized to every reachable binding (`os.environ` class, `numpy.random` module+submodule, `numpy.datetime64`'s rebind disclosed honestly); residual narrows to `ctypes`+pre-capture only, `getrandom()`-via-`Generator(PCG64())` now closed; §5's envelope write ownership corrected (reporting writes `out/`, not the harness). |
