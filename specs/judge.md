...
Version 1.3 · 30 August 2026 · Domain 3 of Requirements v1.2 · References: cross-cutting spec v1.3, worlds spec v1.2, models spec v1.3
...
An outcome "covers" if *all* dimensions fall inside (the joint event is what a user of the prediction experiences; per-dimension coverage is also recorded for diagnosis). Coverage is computed separately in-region and out-of-region (WD-5 labels), **and separately per task (design-review-003 fix — Round 3 found `calibration` was the one metric block left flat despite ADR-J4 defining its own 200-independent-trial set per (model, world, region, *task*); two tasks per world means two genuinely different trial populations, which a single scalar coverage number cannot represent)**, over the independent-trial set only (§ADR-J4 sampling). The diagnostic and the ADR-J1 summary are distinct fields in the verdict and never combined (JU-4's "never conflate", TC-JU4-01).
...
**Decision:** Sharpness = mean over trials and dimensions of the stated 90% central-interval width (2·z₀.₉₀·σ, using the z₀.₉₀ = 1.6449 constant from ADR-J2), in normalised units, **per region and per task (design-review-003 fix — ADR-J4's own hedging cross-check reads "that region/*task's* mean 90%-interval width," which a region-only sharpness block cannot supply; tasks at very different tolerances, e.g. LV's τ=0.10 control vs τ=0.40 planning, produce very different stated interval widths from the same model)**. Smaller is sharper; it is monotone in stated σ by construction (TC-JU5-02's property). Reported in the same verdict block as calibration so neither is readable without the other (GBR: maximise sharpness subject to calibration).
...
```json
{
  ...
  "calibration": {"per_task": [{"task": "lv-control", "levels": [0.5, 0.8, 0.9, 0.95],
    "coverage_in_region": [0.52, 0.79, 0.9, 0.94],
    "coverage_out_region": [0.44, 0.71, 0.82, 0.88],
    "n_trials": 200,
    "per_dimension": "..."}]},
  "sharpness": {"per_task": [{"task": "lv-control",
    "mean_width_90_in": 0.18, "mean_width_90_out": 0.21}]},
...
**`calibration` and `sharpness` are now keyed per task (design-review-003 fix — Round 3 found both were the one metric family left flat, contradicting ADR-J4's own per-task-and-region 200-trial-set definition, and, for sharpness, contradicting ADR-J4's own hedging cross-check text, which already read "that region/task's" mean width):** both blocks restructured from a single flat object to a `per_task` array, matching the shape every sibling block (`skill`, `exceptions`, `trials`, `climatology`, `trust_horizons`) already used. **`climatology.per_task` now carries a `region` field**, since `switch_step` is explicitly defined against "the trial's region's curve" (ADR-J5) and divergence curves differ materially by region.
