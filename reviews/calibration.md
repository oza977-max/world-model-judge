...
**Round 7 fixes applied (31 Aug 2026 — specs → v1.7, test-cases → v1.4; the user chose "fix all with execution-verification" and delegated the C1 design decision).** Every enforcement-mechanism fix was written as a python3 script and RUN before the spec text, result cited (the BC-2 discipline, held again).
- **The BC-4 design decision (delegated to Claude, reversible in the spec text):** JU-12's *completeness* claim is **dropped**. The runtime purity harness is reframed (cross-cutting ADR-002 rule 3, judge §4, architecture-overview §2) as a **best-effort tripwire over the known ambient-capability surface**, with the **ten-run byte-identity gate TC-NF1-01 as the load-bearing reproducibility backstop**, and the `ctypes`/`getrandom` residual **disclosed** (TC-JU12-04). Rationale recorded: (a) it is the honest move for a project whose own thesis (JU-10) is disclosing limits; (b) out-of-process seccomp sandboxing was **rejected** because it is Linux-only and would break NF-1's cross-machine byte-identity — trading the load-bearing guarantee for a secondary one; (c) NF-1 byte-identity already catches the *accidental* case (a `getrandom()`-backed or `numpy.random` slip varies run-to-run). Two accidental routes were also hardened and executed: `os.environ` guarded by mutating the real `_Environ` methods in place (not a rebind — closes the pre-capture escape); all 63 public `numpy.random` module-level names patched (not just `Generator`/`default_rng` — closes `np.random.normal()`).
- **Mechanical cluster, all executed:** judge §4 purity *body* rewritten to mutate-in-place (it had carried the disproven `sys.modules`-swap wording through v1.6 — only the changelog had claimed the fix); TC-JU12-03 and TC-MU7-02 actually wired into `[Test:]` tags, phantom-gate enumeration corrected eight→nine; `models→harness` import gate (TC-NF6-08); `component_key` rejects `:` (TC-NF1-07); TC-MU9-03 pinned to `cli.main(["list-models"])`; `check_prereg` content-invariance (TC-MU6-04, executed with git); `is_baseline` scope stated + `model_ref` cross-roster shift disclosed; harness orchestration loop + JudgeInput per-region partitioning pinned as **cross-cutting ADR-004** (TC-NF1-05/06, open since Round 5); SVG byte-identity pins `svg.hashsalt`+`metadata Date:None` (TC-RP7-02, executed); run-manifest `package_versions` given a named `importlib.metadata` source; the two missing v1.6 changelog rows added. Eight new cases (97 IDs, 96 live); all banners → v1.7.

**Self-verified within the session by execution, not prose — but BC-2 says an independent adversarial re-run is still owed. A Round 8 (dual/blind, executable) is the place to confirm these hold, especially the reframed purity claim and ADR-004, which no independent panel has yet seen.** Committed `db4a15e`.
