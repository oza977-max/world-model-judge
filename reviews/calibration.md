...
| Dimension | R1 | R2 | R3 | R4 | R5 | Trend |
|---|---|---|---|---|---|---|
| Requirements Coverage (Panel A) | 8/10 | 7/10 | 6/10 | 4/10 | 6/10 | ↑ — BC-3's reconciliation genuinely held (79/78 IDs, ten-orphan wiring, tombstone all independently re-derived correct); but 3 of the 10 wirings are defectively placed, so "reconciled ≠ correct" |
| Interface Contracts (Panel B) | 4/10 | 5/10 | 4/10 | 3/10 | 4/10 | → — canonical keying table matches its JSON in every row but calibration/sharpness (BC-1 recurs there); every `.html` twin 3 rounds stale |
| Structural Soundness (Panel C) | 5/10 | 4/10 | 4/10 | 3/10 | 4/10 | → — WorldContext dissolves the R4 mutual exclusion, but the same channel omits the scale vector direct/ensemble need and there is no training/fit entry point at all |
| Implementability (Panel D) | 4/10 | 5/10 | 6/10 | 5/10 | 4/10 | → — critical path and 28-chunk count independently confirmed correct (first time), but 3 of the 10 newly-wired cases can't be satisfied in their chunk |
| Security (Panel E) | — | 6/10 | 5/10 | 4/10 | 2/10 | ↓↓ — allowlist closed the "unlisted module" class, but the gate is defeated 3 ways by Python reflection (executed live) and the named runtime backstop is blind to 2 of them |
| Reproducibility (Panel F1) | — | 5/10 | 4/10 | 5/10 | 5/10 | → — repair held on pkgutil/PNG/trial-sharing, but both panels executed a new critical: no pinned seed-derivation behind ADR-002 rule 2 |
| Separability (Panel F2) | — | 7/10 | 3/10 | 2/10 | 3/10 | ↑ slightly — WorldContext genuinely dissolves the R4 mutual exclusion, but TC-MU9-01 v3 now corrupts the in-process registry (disk leak fixed, in-process leak introduced) |
