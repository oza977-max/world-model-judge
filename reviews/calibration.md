...
**Round 2 fixes applied (30 Aug 2026):** the user chose "fix everything before build" a second time. All 8 Critical and 9 Important Round 2 findings were patched directly into the specs (now v1.2) in the same session, by the same agent that synthesised them — **not independently re-verified**, the identical honesty gap this file named after Round 1 and that Round 2 itself was run to close. Fixed: the double-pendulum EOM coefficient error (worlds.md, with the invalidated TC-WD1-01 reference constant flagged for recomputation, not fabricated); the self-defeating NF-4 scan mechanism (redesigned around a gitignored local terms file with a fail-loud precondition, cross-cutting.md); the AST import-graph gate broadened to block ambient-module imports and dynamic-import calls (the one mechanism two independent Round 2 panels converged on); `trust_horizons.region` and the Chart 4 per-region column split (judge.md, reporting.md); JU-9's mandatory-field list extended to eight groups; `calibration.n_trials`; the `trials.is_exception`/`exceptions.per_task.observed` single-source-of-truth reconciliation; Chart 1's field-name and X-ordering fixes; Chart 2's baseline-curve input clarification; the implementation guide's chunk-count correction (28, not 24) and the P2-C05/P3-C02 file-ownership fix; a named mechanism for TC-MU9-01; a second MU-6 residual-risk disclosure (models.md); a `.md`/`.html` spec-parity check. **Recommended for whoever runs Round 3:** re-verify the double-pendulum fix and the NF-4 redesign specifically, ahead of everything else — a physics formula and a security-mechanism redesign are exactly the kind of change where a confident-looking self-fix is the least trustworthy.
