"""Keyword presence test for `/gvm-requirements` user-guide PP-5 section (TC-PP-5-01 partial).

Pipeline-propagation ADR-605 mandates an "IM-4" section in
`gvm-requirements/docs/user-guide.html`, documenting the Phase 5 finalisation
gates: IM-4 trace gate (discovery ADR-304) and the RA-3 risk-validation gate
(discovery ADR-306).
...
# Supporting keywords proving the section reflects actual IM-4/RA-3 behaviour
# (per SKILL.md and discovery ADR-304/ADR-306).
SUPPORTING_KEYWORDS = (
    "IM-4",
    "RA-3",
    "impact-map.md",
...
def test_pp5_heading_present(guide_text: str):
    assert PP5_HEADING in guide_text, (
        ...
        "gvm-requirements/docs/user-guide.html "
        "(contract: PP5_GUIDE_HEADINGS['gvm-requirements'])"
    )
...
def test_supporting_keyword_present(guide_text: str, keyword: str):
    assert keyword in guide_text, (
        ...
        "must reflect actual IM-4/RA-3 behaviour, not just have the heading"
    )
...
def test_keyword_contract_matches_verifier():
    ...
    assert v.PP5_GUIDE_HEADINGS["gvm-requirements"] == PP5_HEADING, (
        "Drift between this test's PP5_HEADING and the verifier's "
        "PP5_GUIDE_HEADINGS['gvm-requirements'] — fix one of them"
    )
