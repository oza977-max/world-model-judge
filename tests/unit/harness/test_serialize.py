...
def test_canonical_serialize_sorted_keys():
...
def test_canonical_serialize_roundtrips_to_equal_value():
    obj = {"a": [1, 2, 3], "b": {"c": 1.5}}
    out = canonical_serialize(obj)
    assert json.loads(out.decode("utf-8")) == obj


def test_canonical_serialize_is_deterministic_across_calls():
...
def test_canonical_serialize_converts_numpy_array_to_nested_list():
...
def test_canonical_serialize_converts_numpy_scalar_to_native_float():
...
def test_canonical_serialize_no_trailing_whitespace_on_any_line():
...
def test_canonical_serialize_uses_unix_newlines():
    obj = {"a": 1, "b": 2}
    out = canonical_serialize(obj)
    assert b"\r\n" not in out


def test_canonical_serialize_output_is_valid_utf8():
...
def test_canonical_serialize_rejects_infinity():
    with pytest.raises(NonFiniteValueError):
        canonical_serialize({"value": float("inf")})


def test_canonical_serialize_rejects_negative_infinity():
    with pytest.raises(NonFiniteValueError):
        canonical_serialize({"value": float("-inf")})


def test_canonical_serialize_rejects_numpy_nan_inside_array():
